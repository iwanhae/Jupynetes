...
func (tq *TemplateQuery) QueryServer() *ServerQuery {
	query := &ServerQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TemplateQuery) sqlAll(ctx context.Context) ([]*Template, error) {
	...
	if query := tq.withUser; query != nil {
		...
		ids := make(map[int]*Template, len(nodes))
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(template.UserPrimaryKey[1], fks...))
			},
	...
	if query := tq.withServer; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(template.ServerPrimaryKey[1], fks...))
			},
