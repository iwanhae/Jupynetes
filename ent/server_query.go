...
func (sq *ServerQuery) Select(field string, fields ...string) *ServerSelect {
	selector := &ServerSelect{config: sq.config}
...
func (sq *ServerQuery) sqlAll(ctx context.Context) ([]*Server, error) {
	...
	if query := sq.withOwners; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Server, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Owners = []*User{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(server.OwnersPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, sq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "owners": %v`, err)
		}
	...
	if query := sq.withTemplateFrom; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Server, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.TemplateFrom = []*Template{}
		}
		var (
			edgeids []int
			edges   = make(map[int][]*Server)
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(server.TemplateFromPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, sq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "template_from": %v`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "template_from" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.TemplateFrom = append(nodes[i].Edges.TemplateFrom, n)
			}
		}
	}
