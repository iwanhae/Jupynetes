...
func (uq *UserQuery) QueryServers() *ServerQuery {
	query := &ServerQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryTemplates() *TemplateQuery {
	query := &TemplateQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withEvents; query != nil {
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "events": %v`, err)
		}
	...
	if query := uq.withServers; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.ServersPrimaryKey[0], fks...))
			},
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "servers" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Servers = append(nodes[i].Edges.Servers, n)
			}
		}
	}
	...
	if query := uq.withTemplates; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.TemplatesPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "templates": %v`, err)
		}
		query.Where(template.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "templates" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Templates = append(nodes[i].Edges.Templates, n)
			}
		}
	}
